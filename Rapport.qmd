...
![](images/clipboard-3803197578.png)
...
Toutes choses égales par ailleurs, le modèle 1 prédit qu’un individu originaire d'Afrique gagne en moyenne **1,94 \$/h de moins** qu’un Français (soit une pénalité de **17,8 %** selon la spécification en log). Ce résultat est statistiquement significatif au seuil de **1 %**. L'ajout des variables de contrôle (âge, sexe et zone géographique) affine l'estimation, mais le signe, l'ampleur et la significativité de l'effet demeurent **robustes**.

En contrôlant en plus par l'**éducation** (modèle 4), l'écart se réduit légèrement mais reste massif : un individu originaire d'Afrique gagne désormais, toutes choses égales par ailleurs, **1,78 \$/h de moins** qu'un Français (soit **15 %** en moins dans le modèle en log). La persistance de cet effet, même à diplôme identique, suggère qu'une part prépondérante de l'inégalité salariale est directe et ne s'explique pas uniquement par des différences de capital humain. L’ajout de **éducation** provoque en plus une hausse substantielle du $R^2$ (passant de **8,3%** à **25,1%**). Cette progression démontre que le capital humain est le principal facteur explicatif de la variance des salaires dans notre échantillon.
...
#### **c) On ne suppose plus l'exogénéité stricte. Peut-on étudier l'effet de la variable Origine ?**
...
On s’intéresse dans cette partie aux individus actifs (occupés ou chô meurs) avec comme variable dépendante l’indicatrice d’être au chômage. Le statut sur le marché du travail ( actif occupé, actif chômeur, inactif) est disponible chaque trimestre (données de panel avec T=6).

### Question 4 – Modèle de probabilité Linéaire

#### **a) Modèle avec exogénéité des résidus et effets fixes**

```{r}
library(plm)

# 1. Préparation : on garde les actifs et on crée la dummy chômage
df_panel <- df %>%
  filter(acteu %in% c(1, 2)) %>% # 1: Occupé, 2: Chômeur 
  mutate(chomage = if_else(acteu == 2, 1, 0),
         age2 = age^2)

# 2. Estimation du Modèle de Probabilité Linéaire
# On garde la même spécification (X) que pour les salaires
m4a <- plm(
  chomage ~ origine + age + age2 + homme + region + tuu_r + education,
  data = df_panel,
  model = "pooling",
  index = c("id_individu", "t")
)

# Résultats avec écarts-types robustes (clusterisés par individu)
summary(m4a, vcov = vcovHC(m4a, type = "HC2", cluster = "group"))
```

Le résultat le plus frappant concerne **les maghrebins**, qui présente une pénalité massive : à caractéristiques identiques, ces individus ont une probabilité d'être au chômage supérieure de **10,5 points de pourcentage** ($p < 0,001$) par rapport aux français. Pour **les africains**, l'effet est de **5,6 points de pourcentage** supplémentaires. Bien que ce dernier ne soit significatif qu'au seuil de **10 %** ($p = 0,061$), il confirme la tendance observée sur la marge intensive : certaines origines subissent un désavantage non seulement sur le niveau de salaire, mais aussi sur l'accès même à l'emploi.

L'éducation joue un rôle protecteur majeur contre le chômage. Par rapport au diplôme le plus élevé, le risque de chômage augmente systématiquement à mesure que le niveau d'études baisse, atteignant un surplus de **9,9 points de pourcentage** pour ceux n'ayant aucun diplôme ou un CAP ($p < 0,001$). Par ailleurs, le profil de l'âge est **convexe** : la probabilité de chômage diminue avec l'expérience ($\beta_{age} = -0,0088$), mais ce gain de stabilité s'atténue avec le temps ($\beta_{age^2} > 0$), suggérant un retour progressif du risque en fin de carrière. Enfin, les **hommes** ont en moyenne une probabilité plus faible (de **1,45 point de pourcentage)** d'être au chômage par rapport aux femmes, toutes choses égales par ailleurs.

#### **b) Problème si on autorise la corrélation entre effets fixes et regresseurs**

Si on autorise de la corrélation entre effets fixes et régresseurs, alors pour estimer un quelconque effet il faut éliminer les effets fixes (soit par *Within* ou *First diff*). Or, l'origine est de fait un effet fixe; donc dans ce contexte il est impossible d'étudier l'effet de la variable "origine".

### Question 5 – Modèle dynamique et dépendance d'état

#### **a) Crédibilité de l'exogénéité stricte et faible**

Pour capturer la dépendance d’état, nous introduisons le statut passé comme variable explicative. Le modèle s'écrit :

\$\$y\_{it} = \\rho y\_{i,t-1} + X\_{it}\\beta + \\alpha_i + \\epsilon\_{it}\$\$

Où $y_{it}$ est l'indicatrice de chômage, $\alpha_i$ l'hétérogénéité individuelle inobservée et $\epsilon_{it}$ le choc idiosyncratique.

L'exogénéité stricte suppose que $E[\epsilon_{it} | y_{i,1}, \dots, y_{i,T}] = 0$. Or, par construction, le statut courant $y_{it}$ est corrélé au choc contemporain $\epsilon_{it}$ :

\$\$E\[y\_{it} \\epsilon\_{it}\] = E\[(\\rho y\_{i,t-1} + X\_{it}\\beta + \\alpha_i + \\epsilon\_{it}) \\epsilon\_{it}\] = E\[\\epsilon\_{it}\^2\] = \\sigma\_\\epsilon\^2 \> 0\$\$

Puisque le régresseur à la période $(t+1)$ est $y_{it}$, il est de fait corrélé au choc passé $\epsilon_{it}$. Cette corrélation entre un régresseur et les erreurs passées invalide l'hypothèse d'exogénéité stricte.

De façon plus intuitive, l'exogénéité stricte n'est pas crédible ici car un choc comme le Covid en 2020 ($\epsilon_t$) met au chômage immédiatement ($y_{t}$). Ce statut devient mécaniquement le "passé" en 2021 ($y_{t+1}$), servant alors de régresseur pour cette période. Comme l'erreur de 2020 est corrélée au régresseur de 2021, la condition d'indépendance totale sur toutes les périodes est violée par construction.

L'exogénéité faible en revanche est bien plus crédible : elle suppose simplement que le choc Covid de 2020 n'a pas pu influencer le statut d'emploi en 2019, 2018,etc

#### **b,c) Estimation du modèle et interprétation**

Dans le cadre d'un modèle dynamique avec exogénéité faible, les estimateurs First diff et Within sont biaisés vers le bas. Par conséquent, on va recourir à une regression instrumentée en considérant comme instruments $y_{i,t-1}$ puis $y_{i,t-1}-y_{i,t-2}$.

Les résultats de la régression figurent ci-dessous:

```{r}
library(tidyverse)
library(estimatr) # Pour iv_robust et le clustering

df_ah <- df_panel %>%
  arrange(id_individu, t) %>%
  group_by(id_individu) %>%
  mutate(
    # Variables pour l'équation en FD
    d.chomage = chomage - dplyr::lag(chomage),            # Delta y_it
    ld.chomage = dplyr::lag(chomage) - dplyr::lag(chomage, 2), # Delta y_it-1
    
    # Instruments (Anderson-Hsiao)
    instr_level = dplyr::lag(chomage, 2),                 # y_it-2
    instr_diff = dplyr::lag(chomage, 2) - dplyr::lag(chomage, 3),# Delta y_it-2
    
    # Différences pour les contrôles (X)
    d.age = age - dplyr::lag(age),
    d.homme = homme - dplyr::lag(homme) 
  ) %>%
  filter(!is.na(ld.chomage)) # On nettoie les NAs créés par les lags
```

```{r}
# Regression avec y_it-2 comme instrument
fit_ah_level <- iv_robust(
  d.chomage ~ ld.chomage + d.age | instr_level + d.age,
  data = df_ah,
  clusters = id_individu,
  se_type = "CR2",
  diagnostics = TRUE
)

summary(fit_ah_level)
```

Les diagnostics de la régression 2SLS confirment la validité de notre stratégie d'identification. La force de l'instrument (F-stat = 418) garantit une estimation précise, tandis que le test de Wu-Hausman confirme que la dépendance d'état ne pouvait être estimée sans corriger l'endogénéité du statut passé.

Ainsi, toutes choses égales par ailleurs, le fait d'avoir été au chômage au trimestre précédent augmente la probabilité d'être au chômage ce trimestre de **36,4 points de pourcentage.**

```{r}
# Instrumentation par la différence Delta y_{i,t-2}
fit_ah_diff <- iv_robust(
  d.chomage ~ ld.chomage + d.age | instr_diff + d.age,
  data = df_ah %>% filter(!is.na(instr_diff)),
  clusters = id_individu,
  se_type = "CR2",
  diagnostics = TRUE
)
...
L'estimation du modèle dynamique confirme l'existence d'une "trappe à chômage" robuste en neutralisant l'hétérogénéité individuelle inobservée et en corrigeant l'endogénéité du statut passé par l'instrumentation. Le coefficient de dépendance d’état ($\rho$), compris entre **0,20** et **0,36** selon la spécification de l'instrument (différence ou niveau), demeure hautement significatif, validant l'hypothèse d'un ***effet de cicatrice*** pur sur le marché du travail. La pertinence de cette stratégie est corroborée par des statistiques F de première étape massives (allant de **195** à **418**) et un test de Wu-Hausman rejetant sans équivoque l'exogénéité du régresseur retardé.

En somme, ces résultats démontrent que le passage par le chômage réduit intrinsèquement les chances de retour à l'emploi futur, et ce, indépendamment des caractéristiques fixes des individus comme le genre, l'origine ou le talent inobservé.

### Question 6 – Modèle probit ou logit

#### **a) Within et FD pour supprimer les effets fixes**

Les méthodes **Within** et **First-Difference** sont inapplicables aux modèles Probit ou Logit car elles reposent sur des transformations linéaires (centrage ou soustraction) qui ne permettent pas d'isoler l'effet fixe $\alpha_i$ dans un cadre non linéaire.

Dans ces modèles, la probabilité de succès est définie par $P(y_{it} = 1 | X_{it}, \alpha_i) = F(X_{it}\beta + \alpha_i)$, où $F$ est une fonction de répartition non linéaire ( la loi normale $\Phi$ ou la loi logistique). En raison de cette non-linéarité, l'effet fixe n'est pas additif et ne peut être éliminé par une simple différence : $F(X_{it}\beta + \alpha_i) - F(X_{i,t-1}\beta + \alpha_i) \neq F((X_{it} - X_{i,t-1})\beta)$.

De plus, tenter d'estimer directement chaque $\alpha_i$ comme un paramètre supplémentaire conduirait au problème des paramètres incidents, où l'imprécision sur l'estimation des effets individuels avec un $T$ court se propage aux coefficients $\beta$, les rendant biaisés et non-consistants.

#### **b) Estimation des modèles**

```{r}
# On filtre le dernier trimestre
df_last <- df_panel %>% filter(t == 6)

# Estimation Probit
mod_probit <- glm(chomage ~ origine + age + age2 + homme + education + region+ tuu_r, 
                  data = df_last, family = binomial(link = "probit"))

# Estimation Logit
mod_logit <- glm(chomage ~ origine + age + age2 + homme + education + region + tuu_r, 
                 data = df_last, family = binomial(link = "logit"))

# Calcul des effets marginaux moyens (AME)
ame_probit <- summary(margins(mod_probit))
ame_logit <- summary(margins(mod_logit))
```
...
L'estimation des modèles Probit et Logit révèle une forte similarité des effets marginaux moyens, confirmant que les conclusions de l’étude sont robustes à la spécification de la loi utilisée. On observe un désavantage marqué pour les individus d'Europe de l'Est et du Maghreb, dont la probabilité de chômage est supérieure de près de **9 points de pourcentage** à celle des Français. Pour les Africains, bien que l'effet moyen estimé soit positif (environ **+3,7 %**), il demeure non significatif au seuil de **5 %** ($p \approx 0,20$ dans les deux modèles). Cette convergence entre les deux approches renforce le diagnostic d'une barrière à l'emploi sélective.

#### **c) Méthode de Rivers-Vuong pour corriger l'endogénéité**

```{r}
# Méthode de rivers-vuong
# Création de la dummy diplôme supérieur et de l'instrument
df_last <- df_last %>%
  mutate(diplome_sup = if_else(education == "1", 1, 0)) %>%
  mutate(csp_mere_high = case_when(
    # On liste les codes de la "classe instruite"
    csp_mere %in% c("13", "23", "31", "33", "34", "35", "37", "38", 
                    "42", "43", "44", "45", "46", "47", "48", "74", "75") ~ 1,
    # Tout le reste est mis à 0 (classe populaire/prolétaire)
    TRUE ~ 0
  ))

# 1ère étape:  Régression de l'éducation sur les exogènes + Instrument
reg_educ <- lm(diplome_sup ~ origine + age + age2 + homme + region + tuu_r + csp_mere_high, data = df_last)

summary(reg_educ)
```

L’instrument choisi est une variable binaire, **`csp_mere_high`**, qui indique si la mère appartient aux classes instruites ou supérieures (cadres, professions intellectuelles et intermédiaires).

-   **Pertinence :** L'instrument est statistiquement très significatif ($p < 2.10^{-16}$). Avoir une mère issue de la classe instruite augmente la probabilité d'obtenir un diplôme du supérieur de **30,4 points de pourcentage** toutes choses égales par ailleurs.

La 1ère étape étant donc validée, on peut passer à la seconde étape pour estimer le modèle probit.

```{r}
# Récupération des résidus
df_last$residus_educ <- residuals(reg_educ)

# 3. Étape 2 : Probit augmenté (Rivers-Vuong)
mod_rv <- glm(chomage ~ diplome_sup + origine + age + age2 + homme + region + tuu_r + residus_educ,
...
La mise en œuvre de la méthode de Rivers-Vuong ne met pas en évidence d’endogénéité du diplôme du supérieur dans notre modèle : le coefficient du résidu n’est pas significatif ($p = 0{,}297$), ce qui signifie que l’on ne rejette pas l’hypothèse d’exogénéité. Dans ce cas, l’estimation probit standard apparaît appropriée et la correction n’apporte pas de modification substantielle des résultats.

L’enseignement principal de cette estimation réside dans la robustesse de l’effet de l’origine : même en instrumentant le capital humain par la catégorie socio-professionnelle de la mère, les individus originaires du Maghreb (06), d’Europe de l’Est (05) et d’Afrique (07) présentent une probabilité de chômage significativement plus élevée que les Français, toutes choses égales par ailleurs.

Ces résultats doivent toutefois être interprétés avec prudence, dans la mesure où la validité de l’instrument repose sur une hypothèse forte d’exclusion (absence d’effet direct de la CSP maternelle sur l’accès à l’emploi), qui peut être discutée.
```{r}
# Calcul des effets marginaux
ame_rv <- summary(margins(mod_rv))
print(ame_rv)
```

Toutes choses égales par ailleurs, l’analyse des effets marginaux indique que l’origine maghrébine est associée à une probabilité de chômage plus élevée de 10,78 points de pourcentage par rapport à la catégorie de référence (origine française).

Cet écart reste important pour les individus originaires d’Europe de l’Est, avec une augmentation de 7,97 points de pourcentage, tandis qu’il s’élève à 5,69 points pour les personnes d’origine africaine.

Ces résultats suggèrent l’existence d’inégalités d’accès à l’emploi selon l’origine, même en contrôlant pour les caractéristiques individuelles observables.