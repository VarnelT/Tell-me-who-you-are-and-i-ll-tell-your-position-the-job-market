...
meurs) avec comme variable dépendante l’indicatrice d’être au chômage. Le statut sur le marché du travail ( actif occupé, actif chômeur, inactif) est disponible chaque trimestre (données de panel avec T=6).
...
L'éducation joue un rôle protecteur majeur contre le chômage. Par rapport au diplôme le plus élevé, le risque de chômage augmente systématiquement à mesure que le niveau d'études baisse, atteignant un surplus de **9,9 points de pourcentage** pour ceux n'ayant aucun diplôme ou un CAP ($p < 0,001$).
Par ailleurs, le profil de l'âge est **convexe** : la probabilité de chômage diminue avec l'expérience ($\beta_{age} = -0,0088$), mais ce gain de stabilité s'atténue avec le temps ($\beta_{age^2} > 0$), suggérant un retour progressif du risque en fin de carrière. Enfin, les **hommes** ont en moyenne une probabilité plus faible (de **1,45 point de pourcentage)** d'être au chômage par rapport aux femmes, toutes choses égales par ailleurs.
